...
class AccessoryServerBridge():
    _url = f'http://127.0.0.1:9000/'

    def configure(address: str, port: int):
        AccessoryServerBridge._url = f'http://{address}:{port}/'

    def start(request):
        register_key = _get_option(request, 'registerKey', _DEFAULT_KEY)
        options = _get_start_options(request)

        with xmlrpc.client.ServerProxy(AccessoryServerBridge._url, allow_none=True) as proxy:
